...
interface ChatInputContextValue {
	...
	rows?: number;
	onUpload?: (payload: { url: string; fileName: string; fileType: string }) => void;
}
...
// Optional image upload button that integrates with the /api/upload route
interface ChatInputUploadProps extends React.ComponentProps<typeof Button> {
	onUploaded?: (url: string, info: { fileName: string; fileType: string }) => void;
	accept?: string;
}

function ChatInputUpload({ onUploaded, className, accept = "image/*", ...props }: ChatInputUploadProps) {
	...
	const handleFiles = async (file: File) => {
		try {
			...
			if (!presignRes.ok) {
				const err = await presignRes.json().catch(() => ({} as any));
				throw new Error(err?.error || "Failed to get upload URL");
			}

			const { uploadUrl, publicUrl } = await presignRes.json();

			// Step 2: upload the file via PUT to the signed URL
			...
			onUploaded?.(publicUrl, { fileName: file.name, fileType: file.type });
		} catch (e) {
...
ChatInputUpload.displayName = "ChatInputUpload";

export { ChatInputUpload };
