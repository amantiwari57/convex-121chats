...
interface ChatInputUploadProps extends React.ComponentProps<typeof Button> {
  onUploaded?: (
    url: string,
    info: { fileName: string; fileType: string },
  ) => void;
  accept?: string;
}

function ChatInputUpload({
  onUploaded,
  className,
  accept = "image/*",
  ...props
}: ChatInputUploadProps) {
  ...
  const handleFiles = async (file: File) => {
    try {
      ...
      if (!putRes.ok) {
        ...
        throw new Error(
          `Upload failed (${putRes.status}) ${putRes.statusText} ${errText}`,
        );
      }
